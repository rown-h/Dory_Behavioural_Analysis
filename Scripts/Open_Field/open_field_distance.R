# OpenField Distance/Velocity ----

# Requires prior analysis in SimBA (Simon Nilsson).
# Choose 'Analyze Distances/Velocity: Aggregates'
...
# Place Distance and Velocity in two columns of the same row 
of <- data %>%
  pivot_wider(
...
# Standardise distance travelled to account for videos ending prematurely
# Convert to metres
of$distance_standardised <- (of$`Velocity (cm/s)` * videolength)/100 # in metres


# Precompute y-intercepts for horizontal lines
yints <- switch(
  test,
  "Velocity (cm/s)" = seq(0, 40, by = 5),
  "distance_standardised" = seq(
    from = 0,
    to = ceiling(max(of$distance_standardised, na.rm = TRUE) / 50) * 50,
    by = 50
  ),
  stop("test is not acceptable!")  # fails early if test is unexpected
)
...
if (test == "distance_standardised"){
  p <- p +
    ...
    scale_y_continuous(breaks = seq(0, ceiling(max(of$distance_standardised)/50)*50, by = 50))
}
...
# Line and axis styling
 p <- p + theme_minimal() +
  ...
  theme(
    ...
    axis.title.y = element_text(hjust = 13 / 46),
  ...
  # Custom axis lines
  geom_segment(
    data = data.frame(x = 0, xend = 0, y = 0, 
    yend = ifelse(test == "Velocity (cm/s)", 41,
                  ceiling(
                    max(of$distance_standardised, na.rm = TRUE) / 50) * 50+5)),
...
p <- p +
  xlab("Experiment") +
  ylab(ifelse(test == "Velocity (cm/s)", "Average velocity (cm/s)", "Distance (m)")) +
...
# Export graph ----
test_short <- (if (test == "Velocity (cm/s)"){"velocity"}
               else if (test == "Distance (cm)"){"dist-cm"}
               else if (test == "distance_standardised"){"dist-m-stand"}
)
